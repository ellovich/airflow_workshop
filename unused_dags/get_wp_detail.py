...
def get_wp_detail():
    ...
    for wp_id in ids:
        ...
        df = df.T
        df["certification_evaluation_tools"] = df[~df["certification_evaluation_tools"].isna()]["certification_evaluation_tools"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df["discipline_sections"] = df[~df["discipline_sections"].isna()]["discipline_sections"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df["outcomes"] = df[~df["outcomes"].isna()]["outcomes"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df["bibliographic_reference"] = df[~df["bibliographic_reference"].isna()]["bibliographic_reference"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df["prerequisites"] = df[~df["prerequisites"].isna()]["prerequisites"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df["structural_unit"] = df[~df["structural_unit"].isna()]["structural_unit"].apply(
            lambda st_dict: json.dumps(st_dict)
        )
        df = df[target_fields]
        postgres_hook.insert_rows("stg.wp_detail", df.values, target_fields=target_fields)
