...
t as (
    select 
        ...
        u.id as up_id,
        wu.wp_id 
    from dds.up u join dds.levels l on u.level_id = l.id 
...
t2 as (
    select 
        ...
        w.wp_title,
        case 
            when (w.wp_description is null) then 0
            else 1 
            end as description_flag,
        s.state_name 
    from t join dds.wp w on t.wp_id = w.wp_id 
...
t as (
    select 
        ...
        u2.unit_title,
        case 
            when (wp.wp_description is null) then 0
            else 1 
            end as description_flag,
        we.editor_id
    from dds.wp wp join dds.states s on wp.wp_status = s.id
...
t as (
    ...
    from dds.up u left join dds.wp_up wu on wu.up_id = u.id 
        left join dds.wp w on w.wp_id = wu.wp_id 
        left join dds.states s on w.wp_status = s.id
        left join dds.units u2 on u2.id = w.unit_id
        left join dds.wp_editor we on w.wp_id = we.wp_id
        left join dds.wp_markup wm on w.discipline_code = wm.discipline_code 
...
t as (
    ...
    from dds.up u left join dds.wp_up wu on wu.up_id = u.id 
        ...
        left join dds.wp_editor we on w.wp_id = we.wp_id
        left join dds.wp_markup wm on w.discipline_code = wm.discipline_code 
        left join dds.levels lev on u.level_id = lev.id 
        left join stg.evaluation_tools et on w.wp_id = et.wp_id 
...
select
    ...
    t.tool_type,
    t.tool_id
from t left join dds.units u2 on u2.id = t.up_unit
...
t1 as (
    select 
        ap_isu_id, 
        title as op_title, 
        work_programs ::json->>'id' as disc_id  
    from stg.disc_by_year dby
),
t2 as (
    select 
        id::text,
        ...
        certification_evaluation_tools,
        case 
            when expertise_status  ='AC' then 'одобрено' 
