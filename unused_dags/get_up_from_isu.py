...
def get_up_isu():
    ...
    while start < len(ids):
        ...
        for up_id in ids[start:finish]:
            ...
            print(page)
            df = pd.DataFrame(page.json()["result"])
            df["disciplines_blocks"] = df[~df["disciplines_blocks"].isna()]["disciplines_blocks"].apply(
                lambda st_dict: json.dumps(st_dict)
            )
            if len(df) > 0:
                PostgresHook(postgres_conn_id="PG_WAREHOUSE_CONNECTION").insert_rows(
                    "stg.up_isu", df.values, target_fields=df.columns.tolist()
                )
        start += 100
        finish = start + 100
...
with DAG(
    ...
    t1 = PythonOperator(task_id="get_up_isu", python_callable=get_up_isu)
